...
st.title("Randomized Student Assessment Mark Allocator (Flexible Scheme)")
...
if scheme_file:
    ...
    if records_file:
        ...
        if assess_missing:
            st.error(f"These assessment columns are missing in student file: {assess_missing}")
            st.stop()
        if "Total Marks out of 100" not in stu_df.columns:
            st.error("Missing column: 'Total Marks out of 100' in student file.")
            st.stop()

        def marks_given_total(total, N, minmarks, maxmarks, weights, maxmarks_vec):
            # Solve by random search first, then LP if no solution found
            minfrac = minmarks / maxmarks_vec
            maxfrac = np.ones_like(minfrac)
            for _ in range(1000):
                direction = np.random.dirichlet(np.ones(N))
                w = weights
        ...
        for idx, row in filled_df.iterrows():
            ...
            marks = marks_given_total(
                total, N, minmarks, maxmarks, weights, maxmarks
            )
